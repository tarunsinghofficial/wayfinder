...
import { getValidSurveys, getValidStopSurvey, getShowSurveyOnAllStops, getMapSurvey, submitHeroQuestion, updateSurveyResponse } from '../../lib/Surveys/surveyUtils';
...
const mockResponseData = { survey_response: { id: "1" } };
    ...
    const surveyResponse = { id: 1, user_identifier: 'user123', stop_identifier: 'stop456',
                           "responses": "[{ question_id: 1, question_label: 'Question', question_type: 'radio', answer: 'yes' }]" };
...
describe('updateSurveyResponse', () => {
  ...
  it('should throw an error when response is not ok', async () => {
    ...
    await expect(updateSurveyResponse(surveyPublicIdentifier, surveyResponse))
      .rejects.toThrow('Failed to update survey response');
  });
});
